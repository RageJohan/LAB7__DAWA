// Importamos el módulo 'jsonwebtoken' para trabjar con JWT
import jwt from "jsonwebtoken";

//Importamos los modelos y constantes definidos en la aplicacion
import db from "../models/index.js";

//Importamos la configuracion de autenticacion (clave secreta)
...
export const verifyToken = async (req, res, next) => {
    ...
    if (!token) {
        return res.status(400).json({ message: "!No se proporciono un token!" });
    }
    
    try {
        //Verificamos y decodificamos el token, eliminando el prfijo 'Bearer' si existe
...
/**
 * Middleware para verificar si el usuario tienen el rol de 'admin'
 */
export const isAdmin = async (req, res, next) => {
    try {
        //Buscamos al usuario en la base de datos utulizando el ID almacenado en la solicitud
        const user = await User.findByPK(req.userId);
        ...
        const adminRole = roles.find((roles) => role.name === "admin");
        ...
        res.status(403).json({ message: "!Se require el rol de administrador!" });
    } catch (error) {
...
export const isModerator = async (req, res, next) => {
    try {
        //Buscamos al usuario en la base de datos utilizando el ID almacenando en la solicitud
        const user = await User.findByPK(req.userId);
...
export const isModeratorOrAdmin = async (req, res, next) => {
    try{
        ...
        const hasRole = roles.some((roles) => ["admin", "moderator"].includes(role.name));
        ...
        res.status(403).json({ message: "!Se require el rol de moderador o administrador!" });
    } catch(error) {
        //Si ocurre un error, respondemos con un error 500
        res.status(500).json({ message: error.message });
    }
